#https://adventofcode.com/2021/day/16

import logging

#logging.basicConfig(level=logging.DEBUG)

def hex_to_bin_str(hex_str):

    val = int(hex_str, 16)
    bin_str = str(bin(val))[2:] # remove 0b
    
    #pad zeros. Length of bin_str must be a multiple of 4.
    l = len(bin_str)
    x = l % 4
    y = l // 4
   
    pd = 0
    if x > 0: # need padding
        pd = (4* (y+1)) - (l)
    
    # The zfill() method adds zeros (0) at the beginning of the string, until it reaches the specified length.
    bin_str = bin_str.zfill(l+pd)

    logging.debug('*' * len(hex_str))
    #logging.info(f'{l=} {x=} {y=} {pd=} {bin_str=}')
    logging.debug(hex_str)
    logging.debug('*' * len(hex_str))

    return bin_str   


def parse_packet(p):
    

    logging.debug(f"\nProcessing {p=}")
    if len(p) == 0:
        return
    
    ver = int(p[0:3], 2)
    typId = int(p[3:6], 2)


    ii = 6
    if typId == 4:
        # Literal Packets
        next5 = p[ii:ii+5]
        numStr = ""
        while len(next5) == 5:
            logging.debug(f'{next5=} {ii=}')
            numStr = numStr + next5[1:]
            ii = ii + 5 # Get ready for next iteration
            if next5[0] == '0':
                break
                
            next5 = p[ii:ii+5]

        numInt = int(numStr,2)   # length of the sub-packets
        logging.debug(f'Literal Packet processed {numInt=} {ii=} remaining packet {p[ii:]=}')
        return (p[ii:], numInt)
    else:
        # Oper Packets       
        ltid = int(p[ii], 2)
        ii = ii + 1 
        
        leftOverPacket = None
        operands = []
        

        if ltid == 0: # 0 means accumulate next 15 bits, 1 means 11 bits.
            nextGrp = p[ii: ii+15] # 15 bits contain the length of the sub-packets
            ii = ii+15
            len_of_all_subpkts = int(nextGrp, 2)
            sub_pkt_grp = p[ii: ii+len_of_all_subpkts]
            ii = ii + len_of_all_subpkts
            logging.debug(f'Oper Packet Length based {ver=} {typId=} {ltid=} {len_of_all_subpkts=} {sub_pkt_grp=}')
            leftOverPacket = p[ii:] # subpackets will be processed. Everything else should be returned back.
            p = sub_pkt_grp
            
            while len_of_all_subpkts > 0:                           
                lb4 = len(p)
                p, o = parse_packet(p)
                operands.append(o)
                laftr = len(p)
                len_of_all_subpkts = len_of_all_subpkts - (lb4 - laftr)
                logging.debug(f'Oper Packet Length next packet {lb4=} {laftr=} {ii=} {len_of_all_subpkts=}')
            

            #return (leftOverPacket, operResult)

        else:
            nextGrp = p[ii:ii+11]
            ii = ii+11
            num_of_pkts = int(nextGrp, 2) 
            p = p[ii:]
            logging.debug(f'Oper packet count based {ver=} {typId=} {ltid=} {num_of_pkts=} {p=}')

            for _ in range(num_of_pkts):
                lb4 = len(p)
                p, o = parse_packet(p)
                operands.append(o)
                laftr = len(p)
                ii = lb4 - laftr
                logging.debug(f'Oper Packet count processing {lb4=} {laftr=} {ii=} next is {p=}')
        
            leftOverPacket = p

        # Now Perform Operation
        operResult = 0
        if typId == 0:
            operResult = sum(operands)
        elif typId == 1:
            operResult = 1
            for mm in operands:
                operResult = operResult * mm
        elif typId == 2:
            operResult = min(operands)
        elif typId == 3:
            operResult = max(operands)                
        elif typId == 5:
            if operands[0] > operands[1]:
                operResult = 1
        elif typId == 6:
            if operands[0] < operands[1]:
                operResult = 1
        elif typId == 7:
            if operands[0] == operands[1]:
                operResult = 1

        logging.debug(f'Oper Packet Length operation results {operands=} {typId=} {operResult=}')

        return (leftOverPacket, operResult)

    raise AssertionError(f'Unreachable Code - Should have returned before')


if __name__ == '__main__':
    
    sampleHex = [
        ('C200B40A82', 3),
        #('04005AC33890', 54),  # Won't work. Bug in AOC example.
        ('880086C3E88112', 7),
        ('CE00C43D881120', 9),
        ('D8005AC2A8F0', 1),
        ('F600BC2D8F', 0),
        ('9C005AC2F8F0', 0),
        ('9C0141080250320F1802104A08', 1),
        ('', 834151779165)
    ]

    for testval, expAns in sampleHex:
        _, result = parse_packet(hex_to_bin_str(testval))
        print(f'{testval=} {result=} {expAns=}')
